...
const filePath = path.join(process.cwd(), 'migracion_ventas_productos_saldos.xlsx');

console.log('🔍 ANÁLISIS DETALLADO DEL ARCHIVO DE MIGRACIÓN - VENTAS');
...
try {
  ...
  const analysis: Record<string, any> = {
    metadata: {
      fileName: 'migracion_ventas_productos_saldos.xlsx',
    ...
    columns.forEach((col, i) => {
      ...
      const types = [...new Set(sampleValues.map((v: any) => {
        ...
        if (typeof v === 'string') {
          ...
          if (v.match(/^\d{4}-\d{2}-\d{2}/) || v.match(/^\d{2}\/\d{2}\/\d{4}/)) return 'date';
          return 'string';
        }
        return typeof v;
      }))];
      ...
      if (sampleValues.length > 0) {
        console.log(`       - Ejemplos: ${sampleValues.slice(0, 3).map((v: any) => String(v).substring(0, 40)).join(', ')}`);
      }
    });
  ...
  const outputPath = path.join(process.cwd(), 'analisis_migracion_ventas_completo.json');
  ...
  if (error.code === 'MODULE_NOT_FOUND') {
    ...
    console.error('   Luego ejecuta: npx tsx scripts/analizar-migracion-ventas.ts');
  }
  process.exit(1);
}
